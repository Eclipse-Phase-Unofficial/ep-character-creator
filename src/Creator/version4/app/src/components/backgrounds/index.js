...
const Background = ({background = {}}) => (<Tile>
  ...
  <div>
    {_.map(
      background.traits,
      t => <TooltipDefinition key={t.atomUid} tooltipText={t.description}>{t.name}</TooltipDefinition>)}
  </div>

  <h6>Bonus & Malus</h6>
  <div>
    {_.map(background.bonusMalus,
      bm => bm.name)}
  </div>

  <h6>Limitations</h6>
  <div>
    {_.map(background.limitations,
      bm => bm)}
  </div>
...
const mapStateToProps = state => ({
  background: state.data.backgrounds.data[state.data.backgrounds.selected]
})
