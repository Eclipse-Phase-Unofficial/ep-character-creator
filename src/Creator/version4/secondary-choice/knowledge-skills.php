			...
			<?php		         
		         ...
		         foreach($_SESSION['cc']->getKnowledgeSkills() as $m){
		        	...
		        	if($prefix != null || $prefix != ""){
			        	echo "		<td class='skName' id='$id' atomic='$id'><div class='spezBox' id='spezBox".$id."'><input class='spezInt' type='text' id='spe_".$id."' /></div> ".$prefix." : ".$m->name;
		        	}
		        	else{
		        		echo "		<td class='skName' id='$id' atomic='$id'><div class='spezBox' id='spezBox".$id."'><input class='spezInt' type='text' id='spe_".$id."' /></div>".$m->name;
		        	}
		        	...
		        	echo "		<td><input class='knoskillbase' type='number' atomic='$id' min=0 step=5 value='".$m->baseValue."'/></td>\n";
		        	echo "		<td>".$m->linkedApt->abbreviation."</td>\n";
/* 		        	echo "		<td>".$m->morphMod."</td>"; */
/* 		        	echo "		<td>".$other."</td>"; */
		        	echo "		<td id='skillTotalCol'>".$m->getValue()."</td>\n";
		        	if($m->tempSkill){
		        		echo "		<td><span class='icone remKnowSkill' atomic='$id' data-icon='&#x39;'></span></td>\n";
		        	}
		        	else{
			        	echo "		<td></td>\n";
		        	}
