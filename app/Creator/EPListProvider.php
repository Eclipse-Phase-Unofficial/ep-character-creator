...
class EPListProvider {
    ...
    {
        $gearList = array();
        $gearRes = self::$database->query("SELECT `name`, `description`, `type`, `cost`, `armorKinetic`, `armorEnergy`, `damage`, `armorPenetration`,`JustFor`, `unique` FROM `gear`");
    ...
    function getGearByName(string $name): EPGear
    {
        ...
        $gRes = self::$database->query("SELECT `name`, `description`, `type`, `cost`, `armorKinetic`, `armorEnergy`, `damage`, `armorPenetration`,`JustFor`, `unique` FROM `gear` WHERE `name` = '".$this->adjustForSQL($name)."';");
        ...
        $gear = new EPGear($gearRow['name'], $gearRow['description'], $gearRow['type'], intval($gearRow['cost']),
            ...
            intval($gearRow['armorPenetration']), $bonusMalusGearList, $gearRow['JustFor'], $gearRow['unique'] !== "N");
        return $gear;
    }
