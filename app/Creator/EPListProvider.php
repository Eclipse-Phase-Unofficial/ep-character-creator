...
class EPListProvider {
    ...
    {
        $res = self::$database->query("SELECT `name`, `desc`, `type`, `target`, `value`, `targetForChoice`, `typeTarget`, `isCostModifier`, `requiredSelections` FROM `bonusMalus` WHERE `name` = '" .$this->adjustForSQL($name)."';");
